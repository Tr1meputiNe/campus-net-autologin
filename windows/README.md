...
计划任务**不是**直接启动 `powershell.exe`，而是通过
`wscript.exe run-hidden.vbs "<脚本路径>"` 启动 —— `wscript.exe` 是 GUI 宿主，本身没有控制台，
`WshShell.Run(cmd, 0, False)` 又以窗口样式 0 启动子进程，所以完全看不见。

> 为什么不直接给 `powershell.exe` 加 `-WindowStyle Hidden`？因为 Windows 会**先**创建控制台
> 窗口、**再**交给 PowerShell 处理这个参数，所以每分钟仍会闪一下黑色窗口。

> 如果你的安全软件对 `wscript.exe` 敏感，还有一种方案：把任务改成以服务账户在
> **会话 0** 里运行（"不管用户是否登录都运行"），那种方式天然没有窗口，但需要保存密码或
> 使用 S4U，权限模型也更重。默认不采用。
